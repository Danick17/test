...
**Features:** sign-up with password (scrypt-hashed) and session cookies, log in/out, athlete directory with search and sport filters ranked by Bestmark Score, public profile pages at `/athletes/[handle]`, add, update, and **delete** your own PRs (self-reported PRs display as "logged", seeded official results as "✓ marked"; the ✕ on each row removes it while keeping race history), a **Training Hub** (`/articles`) with 11 PR-improvement guides across all four sports and curated external reading, and an **AI coach** (`/coach`) — a chat that grounds training advice in the logged-in athlete's actual PRs.
...
**AI coach configuration:** set `ANTHROPIC_API_KEY` in the server environment to enable the full coach (Claude Opus 4.8 via the official `@anthropic-ai/sdk`). Without a key, a genuinely functional built-in coach answers instead — a topic-aware engine that recognizes the question (training paces, weekly plan, race-time predictions via Riegel's formula, pacing strategy, swim/bike/tri specifics, strength, tapering, injury) and computes real numbers from the athlete's actual PRs, ending each reply with one concrete next step and linking the relevant Training Hub article. So the coach is useful out of the box and upgrades to Claude automatically once a key is present.
